...
"""
    ...
    for each_row in list_suppliers:
        # парсим построчно на значения полей
        suppliers_id += 1
        company_name = each_row['company_name'].replace("'", "''")
        contact = each_row['contact'].replace("'", "''")
        ...
        row2 += f"""
            UPDATE products  SET suppliers_id={suppliers_id}  WHERE product_name IN ('{products}');"""  # заполнение id

        str_products = ",".join(list_good_products)
        row = f"""
                    INSERT INTO suppliers VALUES ({suppliers_id},'{company_name}','{country}','{state}','{index}','{city}','{phone}','{fax}','{homepage}','{str_products}');"""
        req += row
